...
func (sender *TcpServer) receiveData(clientIndex int) {
	...
	for {
		...
		if err != nil {
			...
			go sender.sendReceiveEvent(buff, len(buff), sender.clientList[clientIndex])
		}
...
func (sender *TcpServer) readDataFormat(clientIndex int) (buff []byte, err error) {
	buff = make([]byte, 1024)
	offset := 0
	defer sender.clientList[clientIndex].Conn.SetReadDeadline(time.Time{})
	...
	for true {
		...
		if err == nil {
			...
			if idx >= 0 {
				...
				sender.chConn <- sender.clientList[idx]
			} else {
...
func (sender *TcpServer) close(clientIndex int) {
	...
	if sender.clientList[clientIndex].Conn != nil {
		...
		go sender.sendConnEvent(sender.clientList[clientIndex].Addr, clientIndex, false)
...
//功能:检测连接有效性
func (sender *TcpServer) checkConnect() {
	count := 0
	for true {
		if sender.checkConn || sender.checkLogin {
			count = 0
			for i := range sender.clientList {
				//连接超时检测
				if sender.checkConn {
				...
				//登录检测
				if sender.checkLogin {
					if !sender.clientList[i].IsLogin {
						sender.close(sender.clientList[i].Index)
					}
				}
			}
